...
@ActiveProfiles("test")
@Transactional
@DisplayName(value = "Project Summary Controller Test Suite")
public class ProjectSummaryControllerTest {
    ...
    void setUp() {
        ...
        checkList1.add(task3);
        checkList1.add(task4);
    ...
    @Test
    @DisplayName("GetProjectSummary: Should return projects summary")
    @WithUserDetails(value = "sam", setupBefore = TestExecutionEvent.TEST_EXECUTION)
    void testGetProjectSummary() throws Exception {

        mockMvc.perform(get("/api/projects/projectsummary/{id}", project1.getId())
                .contentType(MediaType.APPLICATION_JSON))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.projectName", is("Coastal Escape")))
                .andExpect(jsonPath("$.budget", is(
                        23000)))
                .andExpect(jsonPath("$.remainingBudget", is(
                        22480.26)))
                .andExpect(jsonPath("$.length", is(4.5)))
                .andExpect(jsonPath("$.clientName", is("John Moss")))
                .andExpect(jsonPath("$.room", is("KITCHEN")))
                .andExpect(jsonPath("$.status", is("PLANNING")));

    }

    @Test
    @DisplayName("GetProjectSummary: Should return projects summary with no room details")
    @WithUserDetails(value = "sam", setupBefore = TestExecutionEvent.TEST_EXECUTION)
    void testGetProjectSummary_EmptyRoom() throws Exception {

        mockMvc.perform(get("/api/projects/projectsummary/{id}", project2.getId())
                .contentType(MediaType.APPLICATION_JSON))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.projectName", is("Modern Living Room")))
                .andExpect(jsonPath("$.budget", is(
                        6000)))
                .andExpect(jsonPath("$.remainingBudget", is(
                        6000)))
                .andExpect(jsonPath("$.length", is(
                        0.0)))
                .andExpect(jsonPath("$.clientName", is("John Moss")))
